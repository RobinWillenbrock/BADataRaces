...
class BasicBlock:
    def __init__(self, function_name, number, priority, shared_resources=None, successors=None, enable_disable_calls=None, code=None):
    ...
    def __repr__(self):
        return (f"BasicBlock(function_name={self.function_name}, number={self.number}, priority={self.priority}, shared_resources={self.shared_resources}, "
                ...
                f"code={' '.join([line for line, _ in self.code])})")
    ...
    for line in lines:
        ...
        if func_match:
            if bb_num is not None and current_function is not None:
                ...
                blocks[(current_function, bb_num)] = BasicBlock(current_function, bb_num, priority, shared_resources, [], enable_disable_calls, code_lines)
        ...
        for line, line_number in block.code:
            ...
            for resource_name, access_type, res_line_number in block.shared_resources:
                if res_line_number == line_number:
                    resource_accesses[resource_name].append((block.function_name, block.number, access_type, line_number, current_isr_status.copy(), block.priority))
